...
| Allowlisted | `/ping`, `/status`, listings (`/list_new_issues`, `/issues_by_status`, `/list_unassigned_issues`, `/find_issue`, `/top_tickets`, `/new_ticket`), time (`/time_summary`, `/log_time`), memory (`/remember`, `/forget`, `/memory`), LLM (`/summary`, `/ask_issue`, `/note`, `/ol`), audits (`/audit`, `/ca`), `@Ultron` NL routing |
